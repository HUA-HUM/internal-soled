...
import type { ISQLMeliTokenRepository } from 'src/core/adapters/mercadolibre/token/ISQLMeliTokenRepository';
import { MeliTokenDTO } from 'src/core/entitis/mercadolibre/tokens/dto/MeliTokenDTO';
  ...
  private normalizeAndValidate(token: MeliTokenDTO): MeliTokenDTO {
    ...
    if (typeof token.expires_in !== 'number' || token.expires_in <= 0) {
      throw new BadRequestException(
        'expires_in must be a number greater than 0',
      );
    }
